...
{
    [SerializeField] private string towerName; // 방어타워 이름
    [SerializeField] private float range; // 방어타워 사정거리
    ...
    [SerializeField] private LayerMask layerMask; // 움직이는 대상만 타겟으로 지정
    [SerializeField] private Transform tf_TopGun; // 방어타워 포탑
    [SerializeField] private ParticleSystem particle_MuzzleFlash; // 총구섬광 이펙트
    ...
    private void SearchEnemy(){
        ...
        for(int i=0; i<_targets.Length; i++){
            ...
            if(_targetTf.name == "Player"){
                Vector3 _direction = (_targetTf.position - tf_TopGun.position).normalized; // 자기와 상대의 방향
